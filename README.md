...
- 인증키는 URL 경로 세그먼트 방식(`/{KEY}/json/{SERVICE}/...`)으로 전달하면 정상 동작합니다.
  쿼리 파라미터 방식은 별도로 시도하지 않았습니다(경로 방식이 첫 시도에 성공).
...
- YEAR/MONTH 선택 파라미터는 전부 생략, YEAR만 지정, YEAR+MONTH 모두 지정 조합 모두
  정상 동작합니다. `SmartUncomfStatMonth`는 MONTH 파라미터 자체가 없으며 YEAR 단독으로만
  동작합니다(실측 확인).
- API 응답에서 결과가 1건일 때와 여러 건일 때 JSON 구조가 다를 수 있어(단일 dict vs list)
  서버 내부에서 항상 리스트로 정규화합니다.
- 숫자 필드는 JSON에서 실수(예: `55567.0`)로 오며, 아직 지나지 않은 달(`MON_08` 등)은
  빈 태그가 아니라 `0.0`으로 옵니다. 서버에서 모두 정수로 안전 변환하여 반환합니다.
